"""
monitor.py — local metrics monitor.

(SCALPEL dashboard. Disguised name. Binds to 127.0.0.1 ONLY — never
network-accessible. View it from your laptop via SSH tunnel:
...
Why localhost only:
- Red team port-scanning the Pi must NOT find an open dashboard
- Public-facing :8080 would be a giant flashing neon sign
)
"""
...
<style>
  * { box-sizing: border-box; }
  ...
  td.tier1 { color: #5ee07a; }
  td.tier2 { color: #ffc857; }
  td.tier3 { color: #ff7070; }
...
if __name__ == "__main__":
    # CRITICAL: bind to 127.0.0.1, NOT 0.0.0.0
    # Red team must not find this on a port scan.
